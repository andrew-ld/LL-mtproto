...
def _get_schema(resources_path: str) -> Schema:
    tl_path = os.path.join(resources_path, "tl")

    result = Schema()

    with open(os.path.join(tl_path, "system.tl")) as system_tl:
        result.extend_from_raw_schema(system_tl.read())

    with open(os.path.join(tl_path, "application.tl")) as application_tl:
        result.extend_from_raw_schema(application_tl.read())
